...
function getCoverImageStyle(settings: ImageDisplaySettings): CSSProperties {
...
function getPrimaryActionUrl(event: PublicEvent) {
  const registrationLink = event.registration_url?.trim();
  const sourceLink = event.source_url?.trim();

  if (isExternalHttpUrl(registrationLink)) return registrationLink;
  if (isExternalHttpUrl(sourceLink)) return sourceLink;
...
function getRegistrationState(event: PublicEvent): RegistrationState {
  ...
  if (
    ...
    (bookingMethod === "external" ||
      ...
      isExternalHttpUrl(event.source_url))
  ) {
...
function ContactLine({ title, value }: { title: string; value: string }) {
  ...
  if (title === "WhatsApp") {
    ...
    return (
      <a
        href={cleaned ? `https://wa.me/${cleaned}` : "#"}
        target="_blank"
        rel="noreferrer"
