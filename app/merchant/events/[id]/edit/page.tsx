...
function buildInitialFormState(event: EditableEvent): FormState {
  const gallery = getSafeArray(event.gallery_image_urls);
...
export default function MerchantEventEditPage() {
  ...
  function handleAreaChange(nextArea: string) {
    ...
    if (matchedArea) {
      updateForm("district", matchedArea.districtZh);
    }
  }
  ...
  function removeGalleryImage(imageUrl: string) {
    ...
    updateForm(
      "gallery_image_urls",
      form.gallery_image_urls.filter((url) => url !== imageUrl)
    );
  }
  ...
  function buildPayload(nextStatus?: string) {
    ...
    return {
      ...
      language_available: form.language_available.length
        ? form.language_available
        : ["tc"],
      ...
      merchant_confirmed_at:
        nextStatus === "submitted" ? new Date().toISOString() : undefined,
      updated_at: new Date().toISOString(),
    };
  }

  async function saveEvent(nextStatus?: string, redirectToPreview = false) {
    ...
    try {
      ...
      setEvent((current) => {
        ...
        return {
          ...current,
          ...(payload as Partial<EditableEvent>),
            ...
            <div className="relative p-6 md:p-8">
              ...
              <div className="relative">
                <div className="inline-flex items-center gap-2 rounded-full bg-primary-50 px-4 py-2 text-xs font-black text-primary-600">
                  ...
                  HK Family Fun Merchant SaaS Event Builder
                </div>
    ...
    >
      <div className="grid gap-5">
        <TextField
          ...
          required
          value={form.title_tc}
          onChange={(value) => updateForm("title_tc", value)}
          placeholder="例如：Summer Kids Market @ MegaBox 小小檔主・親子創意市集"
        />
        ...
        <TextAreaField
          ...
          onChange={(value) => updateForm("short_description_tc", value)}
          placeholder="一句講清楚活動賣點，會顯示在活動卡片。"
        />
        ...
        <TextAreaField
          ...
          onChange={(value) => updateForm("description_tc", value)}
          placeholder="寫清楚活動內容、亮點、流程、適合家庭、小朋友可參與部分。"
        />

        <TextField
          label="主辦單位"
          value={form.organizer_name}
          onChange={(value) => updateForm("organizer_name", value)}
          placeholder="例如：Test Family Fun Centre"
        />
...
function StepTwoLocation({
  ...
  return (
    <Panel
      ...
      title="Step 2：日期、時間及地點"
      subtitle="讓家長快速知道活動何時、在哪裡舉行"
    >
      <div className="grid gap-5 md:grid-cols-2">
        <TextField
          label="開始日期"
        ...
        <TextField
          ...
          onChange={(value) => updateForm("venue_name", value)}
          placeholder="例如：新城市廣場 / MegaBox / 香港公共圖書館"
        />

        <TextField
          label="Google Map URL"
          value={form.google_map_url}
          onChange={(value) => updateForm("google_map_url", value)}
...
function StepThreePolicy({
  ...
  form: FormState;
  updateForm: <K extends keyof FormState>(key: K, value: FormState[K]) => void;
  toggleArrayValue: <K extends "age_groups" | "language_available">(
    key: K,
    value: string
  ) => void;
}) {
  return (
    <Panel
      ...
      title="Step 3：安全及政策"
      subtitle="清楚列明年齡、風險、退款及天氣安排，減少家長查詢"
    >
      <div className="grid gap-5 md:grid-cols-2">
        <label className="block">
        ...
        <div className="md:col-span-2">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            {AGE_GROUPS.map((age) => (
              <TogglePill
                key={age}
                active={form.age_groups.includes(age)}
                onClick={() => toggleArrayValue("age_groups", age)}
              >
                {age}
              </TogglePill>
            ))}
        ...
        <div className="md:col-span-2">
          ...
          <div className="mt-3 grid gap-3 md:grid-cols-3">
            <CheckCard
              ...
              title="戶外活動"
              note="公園、戶外市集、郊遊"
              onChange={(checked) => updateForm("is_outdoor", checked)}
            />
            <CheckCard
              checked={form.is_water_activity}
              title="水上活動"
              note="泳池、沙灘、水上體驗"
              onChange={(checked) => updateForm("is_water_activity", checked)}
            />
            <CheckCard
              checked={form.is_physical_activity}
              title="體能活動"
              note="運動、攀爬、競技活動"
              onChange={(checked) => updateForm("is_physical_activity", checked)}
            />
          </div>
        </div>
        ...
        <div className="md:col-span-2">
          <TextAreaField
            ...
            onChange={(value) => updateForm("refund_policy", value)}
            placeholder="例如：活動開始前 48 小時可全額退款"
          />
        </div>
        ...
        <div className="md:col-span-2">
          <TextAreaField
            label="改期 / 取消政策"
            rows={3}
            value={form.reschedule_policy}
            onChange={(value) => updateForm("reschedule_policy", value)}
            placeholder="例如：可於活動前 7 天改期一次"
          />
        </div>
        ...
        <div className="md:col-span-2">
          <TextAreaField
            ...
            onChange={(value) => updateForm("weather_policy", value)}
            placeholder="例如：八號風球或黑色暴雨以上自動取消"
          />
        </div>
      </div>
    </Panel>
...
function StepFourBooking({
  ...
  return (
    <Panel
      ...
      title="Step 4：收費、報名及提交"
      subtitle="決定家長怎樣報名、如何聯絡、是否顯示在日曆"
    >
      <div className="space-y-6">
        <div className="grid gap-4 md:grid-cols-4">
          {PRICE_TYPES.map((item) => (
            <button
              key={item.value}
              type="button"
              onClick={() => updateForm("price_type", item.value)}
              className={`rounded-3xl border p-4 text-left transition ${
                form.price_type === item.value
        ...
        <div className="grid gap-5 md:grid-cols-2">
          <TextField
            ...
            onChange={(value) => updateForm("price_min", value)}
            placeholder="例如：0"
            type="number"
          />
          ...
          <TextField
            ...
            onChange={(value) => updateForm("price_max", value)}
            placeholder="例如：120"
            type="number"
          />
        </div>
          ...
          <div className="mt-3 grid gap-4 md:grid-cols-4">
            {BOOKING_METHODS.map((item) => (
              <button
                ...
                type="button"
                onClick={() => {
                  updateForm("booking_method", item.value);
                  updateForm("registration_required", item.value !== "none");
                }}
                className={`rounded-3xl border p-4 text-left transition ${
                  form.booking_method === item.value
                    ? "border-primary-400 bg-primary-50"
                    : "border-slate-200 bg-white hover:bg-slate-50"
                }`}
              >
        ...
        <div className="grid gap-5 md:grid-cols-2">
          <TextField
            ...
            onChange={(value) => updateForm("registration_url", value)}
            placeholder="https://..."
          />
          ...
          <TextField
            ...
            onChange={(value) => updateForm("booking_note", value)}
            placeholder="例如：名額有限，先到先得"
          />
          ...
          <TextField
            ...
            onChange={(value) => updateForm("contact_whatsapp", value)}
            placeholder="例如：5701 8297"
          />
          ...
          <TextField
            ...
            placeholder="例如：info@hkfamilyfun.com"
          />
        </div>
        ...
        <div className="grid gap-3 md:grid-cols-2">
          <CheckCard
            ...
            title="需要預先報名"
            note="會在活動頁提醒家長先報名"
            onChange={(checked) => updateForm("registration_required", checked)}
          />
          ...
          <CheckCard
            ...
            title="室內活動"
            note="適合炎熱或下雨天篩選"
            onChange={(checked) => updateForm("is_indoor", checked)}
          />
          ...
          <CheckCard
            ...
            title="SEN 友善"
            note="只有資料清楚提及時才勾選"
            onChange={(checked) => updateForm("is_sen_friendly", checked)}
          />
          ...
          <CheckCard
            ...
            title="顯示在日曆"
            note="活動日期會出現在日曆功能"
            onChange={(checked) => updateForm("show_on_calendar", checked)}
          />
      ...
      <div className="overflow-hidden rounded-3xl border border-slate-200">
        ...
        <div className="p-5">
          ...
          <p className="mt-3 line-clamp-3 text-sm leading-6 text-slate-600">
            {form.short_description_tc || "活動簡介會顯示在這裡。"}
          </p>
          ...
          <div className="mt-5 space-y-3 text-sm text-slate-700">
            <PreviewLine
              ...
              label="日期及時間"
              value={`${form.start_date || "日期待確認"} ${
                form.end_date ? `至 ${form.end_date}` : ""
              }・${form.start_time || "時間待確認"}${
    ...
    <div className="flex items-center gap-2 text-sm">
      <span
        className={`flex h-5 w-5 items-center justify-center rounded-full ${
          done ? "bg-green-400 text-white" : "bg-white/10 text-slate-500"
        }`}
      >
